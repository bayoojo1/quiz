...
if($count > 0){
  ...
  foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $id = $row['id'];
    $sponsorimg = $row['sponsorsimg'];
    $description = $row['description'];
    // Get the filename from the filepath
    $sponsorImgFileName = pathinfo($sponsorimg, PATHINFO_BASENAME);

    $resultpage .= '<div class="col-sm-6 col-sm-offset-3">';
    $resultpage .= '<div class="panel panel-default optionals">';
    $resultpage .= '<div id="sponsor_'.$id.'" class="panel-heading opt-heading">';
    $resultpage .= $description;
    $resultpage .= '</div>';
    $resultpage .= '<div class="panel-body pbody">';
    $resultpage .= '<img src="../images/sponsors/'.$sponsorImgFileName.'" alt="Image" class="img-responsive center-block">';
    $resultpage .= '</div>';
    $resultpage .= '<div class="panel-footer pfooter">';
    $resultpage .= '<button type="button" id="editsponsor" class="btn btn-success btn-sm" onclick="editsponsor(this);">Edit Description</button>';
    $resultpage .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
    $resultpage .= '<button type="button" id="savesponsor" style="visibility:hidden" class="btn btn-success btn-sm" onclick="save(this);">Save</button>';
    $resultpage .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
    $resultpage .= '<button type="button" id="deletesponsor" style="visibility:visible" class="btn btn-success btn-sm" onclick="deleteBanner(this);">Delete Sponsor Banner</button>';
    $resultpage .= '</div>';
    $resultpage .= '</div>';
    $resultpage .= '</div>';
  }
